...
def features_estimation(signal, channel_name, fs, frame, step, plot=False, verbose=True):
    """
    ...
    """
    VAR = Variance
    RMS = Root Mean Square -- related with the force of the muscle and the on/off activation by setting a threshold like:
    muscle is on if RMS > x * std of the signal
    IEMG = Integrated EMG -- redundant/proportional with RMS
    MAV = Mean Absolute Value
    LOG = Log Detector -- check
    WL = Waveform length
    ...
    MNP = Mean Power
    TP = Total Power
    MNF = Mean Frequency -- related to the muscle fatigue (increase in amplitude and decrease in frequency)
...
def time_features_estimation(signal, frame, step):
    ...
    for i in range(frame, signal.size + 1, step): # the +1 is only to to use it with the already pre-processed signal
        x = signal[i - frame:i]
